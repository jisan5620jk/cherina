...
const ThemeCard = ({ cardIcon, title, description, url, hoverTextColor }) => {
  const hoverTextClasses = {
    coral: "text-PrimaryColor-0",
    Aquamarine: "text-SecondaryColor-0",
    DeepSkyBlue: "text-DeepSkyBlue-0",
  };
  const hoverTextClass = hoverTextClasses[hoverTextColor];

  return (
    <div className="bg-white  pt-[25px] 2xl:w-[272px] pb-5 rounded text-DeepSkyBlue font-Manrope group ">
      ...
      <div className="text-center  mb-9 px-[30px]">
        <h3
          className={`text-lg sm:text-xl leading-[30px] text-darkSlate hover:${hoverTextClass} transition-all ease-in-out duration-300 font-bold`}
        >
          {title}
        </h3>
