    ...
    <div className="rounded-md group">
      <figure className="overflow-x-hidden relative">
        <img
          ...
          draggable={false}
          className={`w-full ${
            color === "gold" ? "border-b-4 border-deepSkyBlue" : ""
          }`}
        />
        <span
          className={`absolute bottom-0 left-0 ${
            color === "aquamarine"
              ...
              ? `bg-DeepSkyBlue`
              : `bg-DeepSkyBlue`
          } px-5 py-1 text-sm leading-[26px] text-white font-medium  z-[1] overflow-hidden rounded-tr-3xl`}
        >
          {category}
        </span>
      </figure>
      <div className="p-7 font-Manrope  shadow-cases rounded-b-[4px]">
        <Link to="/donate">
          <h3
            className={`text-[22px] lg:text-xl xl:text-[22px] leading-[30px] text-darkSlate font-bold hover:text-PrimaryColor-0 transition-all duration-300`}
          >
            {title}
          </h3>
        </Link>
